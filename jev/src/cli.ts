...
const USAGE = [
  ...
  "      [--budget 3.5] [--concurrency 3] [--questions id,id] [--max-cost-per-request 0.5]",
  "      [--resolve retry|skip] [--observed-cost 0.02] [--reply-is-ack-threshold 0.5]",
...
export function parseCommand(argv: string[], env: CliEnv): ParsedCommand {
  ...
  return {
    ...
    options: {
      ...
      concurrency: values.concurrency === undefined ? 3 : num("--concurrency", values.concurrency, "positive-int"),
      ...
      replyIsAckThreshold: threshold === undefined ? 0.5 : num("--reply-is-ack-threshold", threshold, "non-negative"),
    },
  };
}
...
async function main(argv: string[]): Promise<void> {
  ...
  if (outcome.halted !== null) console.log(`予算の上限で停止しました: ${outcome.halted}`);
